...
def create_email():
    try:
        ...
        template = EmailTemplate.objects(admin_uid=admin_uid).first()
        ...
        if not template:
            template = EmailTemplate(admin_uid=admin_uid, all_emails={email_type: EmailData(**email_data)})
        else:
            template.all_emails[email_type] = EmailData(**email_data)

        template.save()

        logger.info(f"Email for type '{email_type}' saved successfully for admin '{admin_uid}'.")
        return jsonify({"message": f"Email saved for {email_type}"}), 200
